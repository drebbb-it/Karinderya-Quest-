...
## Next Steps
Priority build order once resumed:
1. `PlayerController2D` (nothing else is testable without movement)
2. `TimerController` + `LivesController` (the two loss conditions)
3. `TrafficHazard`, `IngredientSnatcher`
4. UI bindings (`InventoryUI`, `TimerUI`, `LivesUI`)
